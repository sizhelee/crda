...
class ReferIt3DNetMix(nn.Module):
    ...
    def forward(self, batch: dict, compute_loss=False, is_test=False, cfg=None) -> dict:
        ...
        student_outs = self.student_model(
            ...
            output_attentions=True, output_hidden_states=True, change=True, cfg=cfg, 
        )

        if compute_loss:
            losses = self.compute_loss(teacher_outs, student_outs, batch, cfg)
            return student_outs, losses
        
        return student_outs

    def compute_loss(self, teacher_outs, student_outs, batch, cfg):
        losses = self.student_model.compute_loss(student_outs, batch, cfg)
